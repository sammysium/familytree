...
from people.forms import AddLocationForm, AddPersonForm, EditPersonForm,AddPhotoForm
from people.models import Location, Person, Marriage, Event, SurnameVariant,Photograph
...
def index(request):
    ...
    regions = Location.objects.raw('''SELECT MIN(l.id) AS id, l.county_state_province AS name, c.name AS country_name,
...
def family_tree_chart(request, person_id):
    person = get_object_or_404(Person, id=person_id)

    return render(request,
                  'people/treechart.html',
                  {'person': person, 'list':json.dumps(list(person.annotated_descendants2()))}) 
...
def add_person_photo(request,person_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_file = Photograph(image = request.FILES['image'])
            new_file.save()
            new_file.people.add(person)
        ...
        return render(request,
                    'people/photo.html',
                    {'name': person.name,
                      'person_id':person_id,   #form.instance.id refused
                     'location_form': AddLocationForm(),
                     'list': Person.objects.select_related('birth')})
